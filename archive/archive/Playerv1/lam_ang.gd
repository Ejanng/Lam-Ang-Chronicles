	...
	if Input.is_action_just_pressed("attack") and not isPassiveCD:
		...
		for body in attackArea.get_overlapping_bodies():
			if Global.playerCurrentAttack and body.has_method("deal_dmg"):
				body.deal_dmg(Global.playerStrength + 100)

		# 🔥 Random self-damage (1 in 5 chance)
		if randi() % 2 == 0:
			var damage = 5
			...
			healthBar.value = Global.playerHealth
			print("💥 Ouch! You accidentally hurt yourself (-", damage, " HP)")
...
func _on_attack_area_body_entered(body: Node2D) -> void:
	if Global.playerCurrentAttack and body.has_method("deal_dmg"):
		body.deal_dmg(Global.strengthBuff + 100)
